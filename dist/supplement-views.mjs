...
export function districtAreaView(asset,{query='',prefecture='all',areaId='',sort='retail_sales_million_yen',dir='desc'}={}){
 ...
 const prefectureCounts=Object.entries(PREF).map(([code,name])=>`${name} ${filterAreas(asset,{prefecture:code}).length}エリア`).join('・');
 ...
 <p class="scope-label">1都3県・${DISTRICT_AREAS.length}エリアの公表地区をまとめた合計です。地区外の店舗を含む地理的な街全域の総額ではありません。集計範囲は地域名をタップして確認できます。</p><p class="note">${prefectureCounts}</p>
 ...
 <div class="area-ranking-controls"><label>並べ替え指標<select id="area-metric">${Object.entries(DISTRICT_METRICS).map(([k,m])=>`<option value="${k}" ${key===k?'selected':''}>${m.label}（${m.unit}）</option>`).join('')}</select></label>${query||prefecture!=='all'?`<button type="button" data-area-reset>条件を解除して全${DISTRICT_AREAS.length}エリアを見る</button>`:''}</div>
 ${!selected?'<div class="empty"><p>この条件のエリア合計はまだ作成されていません。</p><button type="button" data-district-mode="source">公表地区の全データを見る</button></div>':`
 <p class="note" role="status" aria-live="polite">${DISTRICT_AREAS.length}エリア中 ${ranking.rows.length}エリアを表示。${DISTRICT_METRICS[key].label}の順位対象 ${ranking.eligible}エリア／合計不可 ${ranking.rows.length-ranking.eligible}エリア。${dir==='asc'?'小さい順':'大きい順'}で表示中。</p>
