...
export function parseState(hash, data) {
  ...
  return {
    view: ['map', 'profile', 'compare'].includes(p.get('view')) ? p.get('view') : 'map',
...
function lineSelect() {
  return `<div class="select-wrap"><label for="line-select">見る沿線</label><select id="line-select">${state.view === 'map' ? `<option value="all" ${state.line === 'all' ? 'selected' : ''}>8沿線を見渡す</option>` : ''}${DATA.routes.map(r => `<option value="${r.id}" ${state.line === r.id ? 'selected' : ''}>${esc(r.shortName)} · ${esc(r.start)}—${esc(r.end)}</option>`).join('')}</select></div>`;
}
...
function renderCompare() {
  ...
  return `<p class="compare-intro">2〜4沿線を選び、掲載した街の顔ぶれを見比べます。掲載数は調査候補の選び方によって異なり、街の多さ・沿線の充実度を表しません。</p><div class="compare-options" role="group" aria-label="比較する沿線（2〜4本）">${DATA.routes.map(r => `<label><input type="checkbox" value="${r.id}" ${state.compare.includes(r.id) ? 'checked' : ''} ${state.compare.length >= 4 && !state.compare.includes(r.id) ? 'disabled' : ''}><span>${esc(r.shortName)}</span></label>`).join('')}</div>${state.compare.length < 2 ? '<p class="notice" role="status">比較する沿線を、もう1本選んでください。</p>' : ''}<div class="compare-grid">${state.compare.map(id => { const r = route(id), cities = routeCases(DATA, id), tags = [...new Set(cities.flatMap(c => c.features))]; return `<section class="compare-card" style="--route:${r.color}"><h2>${esc(r.shortName)}</h2><p class="endpoint">${esc(r.start)} — ${esc(r.end)}</p><p class="compare-count"><b>${cities.length}</b> このリストの掲載候補</p><ul class="mini-towns">${cities.map(c => `<li><button data-town="${c.id}">${esc(c.name)}</button></li>`).join('')}</ul><p class="reference-features">参考メモに含まれる特徴：<br>${esc(tags.join(' · '))}</p><p class="reference-note">商業規模・中心地間の距離：未算定</p><button class="open-route" data-profile-line="${r.id}">この沿線をたどる →</button></section>`; }).join('')}</div>${state.compare.length >= 2 ? `<aside class="common"><p><strong>選んだ全沿線に共通して掲載する街</strong><br>${common.length ? common.map(c => `<button data-town="${c.id}">${esc(c.name)}</button>`).join(' ') : 'この候補リストにはありません。実際の接続関係を網羅した結果ではありません。'}</p></aside>` : ''}`;
}
function renderDetail() {
  const c = DATA.cases.find(x => x.id === state.town), dialog = $('#detail');
  if (!c) { if (dialog.open) dialog.close(); return; }
  $('#detail-content').innerHTML = `<div class="dialog-head"><p class="eyebrow">TOWN NOTE</p><button class="icon-button" id="close-town" aria-label="街の詳細を閉じる">×</button></div><p class="detail-region">${esc(c.prefectures.join('・'))} · 街候補</p><h2 id="detail-title">${esc(c.name)}</h2><p class="stations">関連する駅：${esc(c.stations.join(' ／ '))}</p><h3>この街を読む手がかり</h3><div class="tags">${c.features.map(t => `<span class="tag">${esc(t)}</span>`).join('')}</div><p class="reference-note">候補選定時の参考メモです。統計で確定した分類ではありません。</p><dl class="metric-grid"><div><dt>商業規模</dt><dd>未算定<small>お店・働く人の集積</small></dd></div><div><dt>交通力</dt><dd>未算定<small>駅の利用・結節性</small></dd></div><div><dt>中心地の境界</dt><dd>未確定<small>点は境界を示しません</small></dd></div><div><dt>推定の信頼度</dt><dd>未評価<small>低評価・0の意味ではありません</small></dd></div></dl><h3>掲載している沿線をたどる</h3><div class="detail-actions">${DATA.routes.filter(r => r.caseIds.includes(c.id)).map(r => `<button style="--route:${r.color}" data-profile-line="${r.id}">${esc(r.shortName)} →</button>`).join('')}</div><p class="reference-note">関連駅は同一駅・改札内乗換を意味しません。<br>参考リスト：2026年8月30日固定</p>`;
...
function render() {
  ...
  $('#page-title').textContent = ({map:'次の街は、どんな街だろう。',profile:'沿線に並ぶ、街の顔ぶれ。',compare:'暮らしの舞台を、見比べる。'})[state.view];
  document.querySelectorAll('[data-view]').forEach(b => b.setAttribute('aria-current', b.dataset.view === state.view ? 'page' : 'false'));
  $('#app').innerHTML = state.view === 'map' ? renderMap() : state.view === 'profile' ? renderProfile() : renderCompare();
  if ($('#line-select')) $('#line-select').onchange = e => navigate({line:e.target.value,q:''});
