...
- Complete rankings: retail sales23/26, food-service sales19/26, personal-service sales12/26, three-sector sales11/26, retail floor22/26. No suppressed/not-applicable districts were removed to complete a rank. 大宮/船橋/松戸 retail remains unavailable. Examples in millionyen: 横浜415030, 川崎174755, 立川157808, 町田143209, 千葉127912, 柏107289. All sales remain calendar2020, floor2021-06-01.
...
- **PUBLISHED v0.10**: https://tokyo-rail-town-atlas.dwdaai.chatgpt.site. Saved version 10 (`appgprj_6a9f389f781c8191b52856216aaa9b32~appgver_f6debb3481348191870c30791453e9a3`), deployment `appgdep_6aa5dba256948191a661b56f46bef7f9` succeeded at 2026-09-12T23:09:33.018213+00:00. Exact deployed source `5fcc9b5f381e21a0e5291e25a7d16b0bc3047887`. All 20 public assets in the 21-file archive match pushed source bytes; normalized hosting configuration matches semantically. This subsequent receipt-only commit changes no public assets.

- Owner requests area totals and ranking. **DEC-0016 authorizes a narrower, reproducible table-defined sum** of the fixed eight source-district collections. This supersedes v0.9's withholding of all parent arithmetic only for that explicitly labeled product. Full geographic town coverage, canonical center membership/boundaries and CoreScale remain unverified and are not authorized by this change.
- New evidence: direct read-only inspection of the official Table 2 workbook, matching SHA256 `44d36113accd458cc8c9c4d5538a187fe72903beaa75002fc705e9f300cc9bb8`. All 12,535 unique district leaf IDs reproduce national establishment/employee totals for retail, food and personal services (six measures); all four target prefectures reconcile; all 1,290 comparisons across 215 leaf-bearing municipality groups reconcile. This establishes empirical table additivity, not a prose exclusivity guarantee or a geographic town inventory. Municipal/prefecture/national total rows have no leaf district ID and are not added to the app's source rows.
- `selected-district-sum-v1`: fixed explicit leaf-ID rosters for 新宿、銀座、池袋、渋谷、日本橋、原宿・表参道、六本木、吉祥寺. Existing opaque browse IDs retained. `selectedDistrictSumAllowed=true` permits the published-row sum; `aggregationAllowed=false` continues to deny a certified whole-center aggregate. All 44 IDs are unique across the eight rosters. Scope descriptions list inclusions/exclusions; original district names, IDs and values remain accessible in the selected area's detail.
- Default regional-data screen now shows **エリア合計・ランキング**. Five metric headers toggle descending/ascending order; ties use 1,1,3. Fixed-column area names remain visible when the metrics scroll. Search selects whole collections and never clips their sum to matching child rows. Source-district table remains a separate mode. URL sort, selected area and filter state persist; horizontal scroll/focus survive rerender.
- Per-metric complete-case sums: all eight have complete retail sales, food-service sales and retail floor area. Personal-service sales and three-sector totals are complete only for 新宿、日本橋、原宿・表参道、吉祥寺. Missing, suppressed, not-applicable, absent or ambiguous source IDs never become zero. Incomplete totals remain null/unranked in either order, with coverage and affected district/metric shown. Duplicate source rows, duplicate roster IDs and cross-area duplicate membership fail closed. All sales use published million-yen rounded observations for calendar2020, shown in 億円; floor uses 2021-06-01. No GDP, current-year economy or full-town coverage claim.
- Existing five public JSON assets and all raw locks remain unchanged. This is browser-side arithmetic on already-published district observations, analogous to the existing complete-case three-sector calculation; the raw-consuming pipeline is not rerun. Read-only workbook reconciliation does not claim `verify-locked` completion. N03, spatial rollups, canonical IDs and source-value files remain untouched.
- Validation: 27 JavaScript checks PASS, including eight new aggregate/rank regressions; `make verify-fast` PASS. Original five public JSON files remain byte-identical, syntax and whitespace checks PASS. Read-only review found one mobile navigation issue; area selection now opens and scrolls to its detail, with a return-to-ranking control. Browser/device QA not requested/performed; no compilation step for this authored static site. Publication succeeded as recorded above.

## Town grouping and comparison-scope correction — 2026-09-12

- **UI PUBLISHED v0.9 / region-total work PARTIAL**: https://tokyo-rail-town-atlas.dwdaai.chatgpt.site. Saved version 9 (`appgprj_6a9f389f781c8191b52856216aaa9b32~appgver_46a3fabe16d081918a960c727ca82f77`), deployment `appgdep_6aa5ced69d308191acb7567b5c7c8c65` succeeded at 2026-09-12T22:14:59.321923+00:00. Exact deployed source `5b53550f90eecc9119692c0f8c4f7032811797b1`. All 20 public assets in the 21-file archive match the pushed source; normalized hosting configuration matches semantically. Read-only review found no further blocker to this provisional navigation design. This receipt-only change does not alter public assets.
